...
describe("edit tool", () => {
  it("replaces oldString with newString in file", () =>
    Effect.gen(function* () {
      ...
      const result = yield* editHandler({ filePath: "/tmp/test-edit.txt", oldString: "world", newString: "universe" })
  ...
  it("returns error when oldString not found in file", () =>
    Effect.gen(function* () {
      ...
      const result = yield* editHandler({ filePath: "/tmp/test-edit2.txt", oldString: "goodbye", newString: "hello" })
      assert.isTrue(result.includes("Error: oldString not found"))
    }).pipe(Effect.provide(testLayer)))

  it("returns error when file doesn't exist", () =>
    Effect.gen(function* () {
      const result = yield* editHandler({ filePath: "/tmp/non-existent-12345.txt", oldString: "test", newString: "new" })
