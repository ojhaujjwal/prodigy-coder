...
# Ralph Loop - Autonomous AI coding agent that implements specs
# Compatible with opencode as the agent backend.
#
...
#
# Examples:
#   ./ralph.sh "Fix the authentication bug in login flow"
#   ./ralph.sh "Implement the exchange rate sync feature" --max-iterations 5
#   ./ralph.sh "Quick experiment" --skip-checks
#   ./ralph.sh "Complex task" --model opencode-go/glm-5.1
#
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --max-iterations)
            ...
            ;;
        --help|-h)
            echo "Usage: ./ralph.sh <focus prompt> [options]"
            echo ""
            ...
            echo "  ./ralph.sh \"Implement exchange rate sync\" --max-iterations 5"
            echo "  ./ralph.sh \"Quick fix\" --skip-checks"
            echo "  ./ralph.sh \"Complex task\" --model opencode-go/glm-5.1"
            exit 0
            ;;
        -*)
...
if [[ -z "$FOCUS_PROMPT" ]]; then
    ...
    echo "Examples:"
    echo "  ./ralph.sh \"Fix the authentication bug\""
    echo "  ./ralph.sh \"Implement exchange rate sync\""
    echo ""
...
extract_task_description() {
    ...
    desc=$(grep "TASK_COMPLETE:" "$output_file" | \
        head -1 | \
...
run_iteration() {
    ...
    if OPENCODE_PERMISSION='{"edit":{"*":"allow",".oxlintrc.json":"deny","lint/**":"deny","tsconfig.json":"deny"},"external_directory":{"/tmp/**":"allow"}}' opencode run -m "$MODEL" -- "$(cat "$prompt_file")" 2>&1 | tee "$output_file"; then
        echo ""
